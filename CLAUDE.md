...
Changing any of these means updating, in lockstep: both Helm charts, both app workflows, the Argo CD Application parameters in `infra/terraform/gitops.tf`, `gitops/values/*.yaml`, `security/secretproviderclass-example.yaml`, and `docs/architecture.md`.
...
**Identity chain (no secrets anywhere).** `infra/bootstrap/bootstrap.sh` creates three Entra apps federated to GitHub OIDC: platform-**ro** (Reader; `pull_request` subject → `infra-plan.yml`, plus a `ref:refs/heads/main` subject → scheduled `infra-drift.yml`), platform-**rw** (Contributor + RBAC Administrator; federated **only** to the `production` environment → `infra-apply.yml`), and **app-ci** (AcrPush only; `ref:refs/heads/main` subject → image push + cosign sign — CI holds **zero** cluster credentials, deploys are pull-based per ADR-0008). Workflow ↔ federated-subject pairing is load-bearing: a job moved out of its expected trigger/environment context will fail OIDC exchange. Separately, pods reach Key Vault via workload identity: user-assigned identity + federated credential per namespace ServiceAccount (`keyvault.tf`) → SecretProviderClass `clientID` → CSI mount + synced Secret.
...
**Pipelines (GitOps, pull-based — ADR-0008).** Path-filtered and independent: `apps/backend/**` and `apps/frontend/**` never trigger each other. Stage order in both: test → docker build (`load: true`, no push) → Trivy gate (HIGH/CRITICAL fails) → main-only: OIDC push of `sha-<gitsha>` → cosign keyless sign of the digest → promote-staging commits `image.tag` to `gitops/values/<app>-staging.yaml` → reviewer-gated promote-production commits the production values file. **CI never runs helm/kubectl** — Argo CD (installed by `gitops.tf`) reconciles git → cluster with prune+selfHeal; rollback = `git revert`. Don't reintroduce push-based deploys. Infra: fmt/validate → Trivy IaC misconfiguration scan (graded) → plan-with-PR-comment on PRs; plan+apply on main behind the `production` environment; nightly read-only drift plan (`infra-drift.yml`) that opens a `drift`-labeled issue. Also: gitleaks on every PR/push (`secret-scan.yml`), Dependabot for all ecosystems, and every action pinned to a commit SHA — keep the `# vX` comment when bumping. All jobs start from `permissions: {}` with per-job opt-in; keep it that way.

**Environments = namespaces.** One cluster, `app-staging` / `app-production`, identical charts. Per-env values live in `gitops/values/*.yaml` (git = what runs); cluster identifiers (ACR, workload-identity client id, Key Vault name, tenant id) are injected into the Argo CD Application specs by Terraform (`gitops.tf`) so no GUID is committed. Kyverno (also Terraform-installed) enforces admission policies from `gitops/policies/` — signature verification (Audit until registry auth is validated), no `:latest`, ACR-only, non-root, resource limits (ADR-0009). Workload placement: apps go to the autoscaled `user` node pool via `nodeSelector: workload=apps`; the `system` pool is tainted critical-addons-only (`aks.tf`).
