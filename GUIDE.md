...
- **TLS**: fully declarative — nothing to install. DNS: Terraform binds a
  **static public IP with an Azure DNS label** to the ingress controller
  ...
  no domain purchase needed, re-claimed identically after the nightly wipe.
  The `letsencrypt-prod` ClusterIssuer is an Argo CD app (`gitops/cert/`);
  the frontend chart already requests a certificate whenever `ingress.host`
  is set. So the only human step is the `FRONTEND_HOST_PRODUCTION` repo
  variable below — after that apply, the certificate issues itself in ~1–2
  minutes and `https://` works.
